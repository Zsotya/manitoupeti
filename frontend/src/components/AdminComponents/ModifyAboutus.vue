<template>
  <div class="modify-aboutus">
    <div class="title">
      <h2>Bemutatkozás elem módosítása</h2>
    </div>
    <!-- Adatmódosítás form -->
    <form @submit.prevent="modifyAboutus">
      ...
      <!-- Véglegesítés gomb -->
      <div class="form-group">
        <button type="submit">Bemutatkozás elem módosítása</button>
      </div>
    </form>
  </div>
</template>
...
onMounted(async () => {
  const aboutusId = route.params.id;
  const response = await axios.get(
    `http://localhost:3000/api/aboutus/${aboutusId}`
  );
  ...
  imagePreviewUrl.value = "http://localhost:3000" + aboutusData.value.image;
});
...
const handleImageUpload = (event) => {
  ...
  if (file) {
    ...
    imagePreviewUrl.value = aboutusData.value.image;
  }
};

// Módosítás
const modifyAboutus = async () => {
  // Form adatok
  ...
  if (imageFile.value) {
    ...
    formData.append("image", aboutusData.value.image); // Ha nem került új fájl kiválasztásra, akkor a régi URL-t kapja
  }
  // PUT request küldése
  try {
    const aboutusId = route.params.id;
    ...
    console.log("Bemutatkozás elem sikeresen módosítva:", response.data);
    router.push("/admin/aboutus-management");
  } catch (error) {
    console.error("Hiba a bemutatkozás elem módosításakor:", error);
  }
};
</script>
